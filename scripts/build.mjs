...
function homeRow(collection) {
  ...
  return `<article class="collection-row collection-${collection.slug}"><a class="collection-panel" href="${href('', collection.slug)}" data-nav-reveal data-cursor="OPEN"><span class="collection-number">${collection.number}</span><div class="collection-name"><h2>${collection.name}</h2>${titleDraw()}<p>${collection.japanese}</p></div><span class="round-arrow" aria-hidden="true">→</span></a><a class="collection-visual" href="${href('', collection.slug)}" data-nav-reveal data-cursor="OPEN" aria-label="${esc(collection.name)}を見る">${visual}<span class="visual-copy">${esc(collection.tagline)}</span></a></article>`;
}
...
function brandPage(collection) {
  const prefix = '../../';
  const a = assets[collection.slug];
  const heroFiles = a.hero;
  const galleryFiles = a.gallery || [];
  for (let index = 0; index < galleryFiles.length; index += 1) {
    collection.captions[index] ||= `${collection.short} GALLERY ${String(index + 1).padStart(2, '0')}`;
    collection.galleryText[index] ||= `${index + 1} / ${galleryFiles.length}`;
  }
